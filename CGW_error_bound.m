function CGW_solution = CGW_error_bound(L, x, n, limit, gamma)
...
figure(1);
txt = ['error for gamma = ', num2str(gamma)];
txt2 = ['bound for gamma = ', num2str(gamma)];
%semilogy(k_vector(1:2:end), err_vector(1:2:end), 'LineWidth', 1, 'DisplayName', txt) % plot every second error for monotone decreasing behaviour
semilogy(k_vector, err_vector, 'LineWidth',1, 'DisplayName', txt)
legend('location', 'east');
hold on

%semilogy(k_vector(2:2:end), err_vector(2:2:end), 'LineWidth',1 , 'DisplayName', txt) % plot every second error for monotone decreasing behaviour
semilogy(k_vector,bound_vector, 'LineWidth',1, 'LineStyle', '--', 'DisplayName', txt2)
